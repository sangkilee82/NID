var siteCode = {
	...
	select: function (oThis, pt_code, depth) {
		...
		$.ajax({
			...
			success: function (data) { siteCode.selectResult(data, depth); }
		});
	...
	selectResult: function (data, depth) {
		...
		if (depth == 3) {
			html += "<td><img src='../images/cog.gif' alt='설정' onclick='FrameLayer.open( \"./regiForm.aspx?code={CODE}\", 500, 300, \"no\" );' /></td>";				 //설정
		}

		html += "<td><img src='../images/action_stop.gif' onclick='siteCode.remove( this );' alt='삭제' /></td>";											             //삭제
		html += "<td onclick='siteCode.select( this, \"{CODE}\"," + select_depth + " );'><img src='../images/icon_accept.gif' class='select_icon' alt='선택' /></td>";	 //선택
		html += "</tr>";
